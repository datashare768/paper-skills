...
> 重新贴一次 `\citep`）；**引用应在各段之间分布均匀，P1（背景）、P2（挑战）不能
> 完全没有引用**，应各配 1-2 条支撑具体论断的引用（不要求与 P3 引用不同的文献，
> 可以是 Related Work 中会用到的综述/经典文献提前引用一次），不能把全部引用都
> 堆在"已有方法综述"P3 段落。
...
- [ ] P1、P2 各自至少包含 1 条 `\citep{}`，不能是空手陈述常识/背景/挑战而完全
      没有引用支撑（引用集中堆在 P3 是常见反例，需要把 1-2 条挪到 P1/P2）
