...
### 13.1 每个 `\subsection`/`\subsubsection` 的段落数量控制

Experiments/Results 讨论部分另一个常见反例是"过度切分段落"：每个小发现单独起一段，
甚至一句话就单独成一段，导致一个 `\subsection` 下堆出 5-6 个只有 1-2 句话的段落，
读起来破碎、缺乏论述节奏。必须遵守：

- **默认段落数量上限为 4，通常应为 3；内容本身较少时用 2 个甚至 1 个即可**，
  具体取决于该 `\subsection`/`\subsubsection` 下有多少个真正独立、值得单独成段的
  论点，而不是机械地"每个子图/每个变体/每个数据集分一段"。
- **禁止一句话单独成段**：如果一段只有 1 句话，先检查它是否应该合并进相邻段落
  （通常是"设置/协议说明句"应该并入紧随其后的结果分析段，而不是自己单独占一段）。
- **合并优先于新增段落**：起草时若发现同一 `\subsection` 下已经写出 4 段以上，
  应先尝试把逻辑上相邻、共享同一个大主题的段落合并（例如"总体结果"与"分数据集
  差异"合并成一段，"两个消融分析各自的设置+结果"各压成一段而不是设置和结果分开
  两段），而不是保留段落数量、只缩短每段文字。
- **合并方法**：把"设置句"改写为结果句的从句/分号连接的并列句（如
  `..., as reported in Table~\ref{tab:xxx}. Removing any single component
  degrades ...` 合并为 `..., and compare them against the full model ... in
  Table~\ref{tab:xxx}; removing any single component degrades ...`）；把同一
  `\subsection` 内两个平行子分析（如"主消融"和"分解策略对比消融"）各自的
  设置句与结果句分别合并成一段，而不是设置单独一段、结果单独一段。
- **例外**：确有 5 个以上互相独立、缺一不可的论点时（如 Parameter Sensitivity
  一次扫 6 个超参数），允许按"主题分组"合并到 3-4 段（例如把架构类超参数、
  偏差量化专属超参数、窗口长度超参数分别归为一段），而不是无脑压成 1 段导致
  段落过长失去可读性，也不是保留 6 个超参数各自 1 段的原始拆分。
- **检查方法**：对每个 `\subsection{}`/`\subsubsection{}`，数一下其下（到下一个
  同级或更高级标题之前）有多少个独立段落（不含 `\begin{table}`/`\begin{figure}`
  环境本身）；超过 4 段或存在只有 1 句话的段落时，按上面的合并方法重写，直到
...
- 对每个 `\subsection{}`/`\subsubsection{}`，按第 13.1 节数一下其下的独立段落
