...
│                               # 防御性辩解性表达/结果段落功能分层/跨章节编号引用/符号与数值书写）
...
去模板化收尾句与机械化段落骨架（消除 AI 写作痕迹）、长句拆分（超 40 词或两个以上
从句必拆）、图表面板交叉引用规范化（禁止裸写 `panel (b)`，须写成
`Fig.~\ref{...}(b)`）、去除防御性辩解性表达（不解释未做之事、不预防性回应假想
质疑、不用辩解性尾句、不自我表扬）、结果段落功能分层（总体结果→差异规律→泛化与
机制归因三段式）、实验章节禁止交叉引用方法章节的章节/公式编号（改用自包含文字
复述）、正文禁止用数学/程序符号代替文字（不写 `A > B > C`/`&`/`→`/`w.r.t.`）
与数值书写全篇一致。
